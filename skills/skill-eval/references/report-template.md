...
Referenced from SKILL.md Step 7. Write as `eval_report.md` in the evaluated skill's `eval/` folder — do NOT modify the skill's `SKILL.md`. Keep it short — a reviewer should grasp the evidence in 60 seconds.

---

```markdown
# Evaluation Report — <skill-name> Skill
...
<ship / fix-and-rerun> — <one sentence why>.

## Limitations

<n> tasks = directional evidence, not statistical. <other caveats,
e.g. one task used synthetic data, judge not yet validated against human labels>.

## Reproduce

Evalset at <path>; runs in experiment `<experiment path>`;
...
- Write the report to `eval/eval_report.md` in the evaluated skill's folder — never modify the skill's `SKILL.md`.
